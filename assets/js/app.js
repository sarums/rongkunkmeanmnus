...
// ── SEO Meta Updater ──
function updateMeta({title, desc, image, url}){
  const sn=siteName||'RongKunKmeanMnus';
  const fullTitle=title?`${title} | ${sn}`:sn;
  document.title=fullTitle;
  const setAttr=(id,attr,val)=>{ const el=$(id); if(el&&val) el.setAttribute(attr,val); };
  setAttr('meta-desc','content',desc||'');
  setAttr('og-title','content',fullTitle);
  setAttr('og-desc','content',desc||'');
  setAttr('og-image','content',image||'');
  setAttr('og-url','content',url||location.href);
  setAttr('og-site','content',sn);
  setAttr('tw-title','content',fullTitle);
  setAttr('tw-desc','content',desc||'');
  setAttr('tw-image','content',image||'');
  setAttr('canonical','href',url||location.href);
}
...
function showPage(id,push=true){
  ...
  if(id==='home') updateMeta({title:null, desc:'Your ultimate destination for curated video content.'});
}
window.goHome=e=>{
  e?.preventDefault();
  showPage("home");
...
window.openVideo=async (id,push=true)=>{
  ...
  // SEO + History
  addToHistory(id);
  updateMeta({ title:v.title, desc:v.description||`Watch ${v.title} on ${siteName}`, image:getThumb(v), url:location.href });
